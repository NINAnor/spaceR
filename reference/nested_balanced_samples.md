...
drawn *from the previous sample only*, but balanced against the auxilary
variables at the level of the whole population, while spatial spreading
is computed based on the coordinated of the included population units
only.
...
- return_indices:
  ...
  Logical; if `TRUE`, also returns row indices (per `samplingFrame`)
  instead of just the filtered data frames. Default `FALSE`.

## Value

A named list. For each `n` in `n_seq` you get an element named `n{n}`
(e.g. `n100`, `n80`, ...), each being a `data.frame` with the rows of
`samplingFrame` that were selected. If `return_indices = TRUE`, each
element is a list with `samplingFrame` and `indices`.
